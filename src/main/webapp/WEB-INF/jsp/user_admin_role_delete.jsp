...
<body>
    ...
    <form class="form-login" action="/admin/user/role/deleteAdmin" method="post">
        <c:if test="${not empty usernameNotExist}">
            <p class="alert alert-danger text-center">${usernameNotExist}</p>
        </c:if>
        <h2>Delete Admin Role</h2>
        <input list="usernames" class="form-control" placeholder="Search user..." name="username">
        ...
        <button class="btn btn-lg btn-block btn-primary" type="submit">Delete</button>
        <input type="hidden"
